...
class TimerApp:
    def __init__(self, root):
        ...
        self.root.configure(bg="#f5f0e1")  # fondo marfil suave

        pygame.mixer.init()

        # === Estilos generales ===
        style = ttk.Style()
        style.configure("TLabel", background="#f5f0e1", font=("Georgia", 12))
        style.configure("TButton", font=("Georgia", 10))
        style.configure("Timer.TLabel", font=("Georgia", 24, "bold"), background="#ffffff", foreground="#333")
        style.configure("Traffic.TLabel", font=("Helvetica", 36), background="#f5f0e1")

        # === Reloj elegante ===
        reloj_frame = tk.Frame(root, bg="#dcd6cc", bd=2, relief="groove")
        reloj_frame.pack(pady=10, fill="x", padx=20)

        self.clock_label = tk.Label(reloj_frame, font=("Georgia", 18), bg="#dcd6cc", fg="#2d2d2d")
        self.clock_label.pack(pady=(5, 0))
        self.date_label = tk.Label(reloj_frame, font=("Georgia", 12), bg="#dcd6cc", fg="#5a5a5a")
        ...
        self.traffic_light.pack(pady=10)

        # Botones de control
        button_frame = tk.Frame(root, bg="#f5f0e1")
        button_frame.pack(pady=10)
        ttk.Button(button_frame, text="Iniciar", command=self.start_timer).pack(side=tk.LEFT, padx=10)
        ttk.Button(button_frame, text="Reiniciar", command=self.reset_timer).pack(side=tk.LEFT, padx=10)

        self.sound_triggered = {"green": False, "yellow": False, "red": False}

    def create_time_picker(self, label, is_start):
        container = tk.LabelFrame(self.root, text=label, bg="#eae7dc", fg="#2f2f2f", font=("Georgia", 12, "bold"))
        container.pack(padx=20, pady=10, fill="x")

        frame = tk.Frame(container, bg="#eae7dc")
        frame.pack(pady=6)

        hour = ttk.Combobox(frame, width=2, font=("Georgia", 11), values=[f"{i:02}" for i in range(1, 13)])
        minute = ttk.Combobox(frame, width=2, font=("Georgia", 11), values=[f"{i:02}" for i in range(0, 60)])
        ampm = ttk.Combobox(frame, width=3, font=("Georgia", 11), values=["AM", "PM"])
        hour.pack(side=tk.LEFT, padx=2)
        minute.pack(side=tk.LEFT, padx=2)
        ampm.pack(side=tk.LEFT, padx=2)

        if is_start:
            self.start_hour, self.start_minute, self.start_ampm = hour, minute, ampm
            tk.Button(frame, text="Ahora", font=("Georgia", 10), command=self.set_now_start, bg="#b0a990").pack(side=tk.LEFT, padx=6)
        else:
            self.end_hour, self.end_minute, self.end_ampm = hour, minute, ampm
            btn_frame = tk.Frame(container, bg="#eae7dc")
            btn_frame.pack()
            for mins in [20, 30, 40]:
                tk.Button(btn_frame, text=f"+{mins} min", font=("Georgia", 10),
                          command=lambda m=mins: self.add_minutes_to_end(m), bg="#b0a990").pack(side=tk.LEFT, padx=5)
